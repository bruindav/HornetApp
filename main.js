...
import './firebase.js?v=610r21f12b';
...
async function loadAdminUsers(){document.getElementById('admin-users').innerHTML='(stub)';}
document.getElementById('admin-refresh')?.addEventListener('click',loadAdminUsers);
// fix12b auth state binding
(function(){
  try{
    const a=getAuth();
    ...
    onAuthStateChanged(a, async(u)=>{
      ...
      try{ const r=await import('https://www.gstatic.com/firebasejs/10.11.0/firebase-firestore.js');
            const { doc, getDoc } = r;
            const dbmod = await import('./firebase.js?v=610r21f12b');
            const dref = doc(dbmod.db, 'roles', u.uid);
            const snap = await getDoc(dref); const data = snap.exists()? snap.data() : {};
            const nl=data.rol; CURRENT_ROLE=(nl==='beheerder'?'admin':(nl==='vrijwilliger'?'volunteer':'guest'));
            ALLOWED_ZONES=Array.isArray(data.gebieden)? data.gebieden:[];
