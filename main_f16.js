import { getFirestore, collection, getDocs, doc, getDoc, setDoc, deleteDoc } from 'https://www.gstatic.com/firebase_f16.js/10.11.0/firebase_f16.js';

import { getAuth, onAuthStateChanged, GoogleAuthProvider, signInWithPopup, signOut } from 'https://www.gstatic.com/firebase_f16.js/10.11.0/firebase_f16.js';
import './firebase_f16.js?v=610r21f16';
import './sync_engine_f16.js?v=610r21f16';
// app.js — Hornet Mapper NL v6.1.0 (hybride realtime + veilige UI binding)
// ----------------------------------------------------------------------------
// Vereist (door index.html alléén app.js te laden):
// ./sync_engine_f16.js → importeert ./firebase_f16.js → importeert ./config_f16.js
...
// fix12b: inject login UI and role indicator if missing
...
async function adminRefresh() {
  try {
    ...
    const items = [];
    snap.forEach(d => { const data=d.data()||{}; items.push({ uid:d.id, rol:data.rol||'(geen)', gebieden:Array.isArray(data.gebieden)?data.gebieden:[] }); });
    const list = document.getElementById('admin-list');
    if (list) list.innerHTML = items.length? items.map(it => `
      ...
      </div>`).join('') : '<i>Geen rollen gevonden.</i>';
    list?.querySelectorAll('.adm-row').forEach(row => {
      row.addEventListener('click', async () => {
        ...
        document.querySelectorAll('.adm-area').forEach(cb => { cb.checked = Array.isArray(d.gebieden)? d.gebieden.includes(cb.value) : false; });
      });
    });
  } catch(e) { console.error('[adminRefresh]',e); alert('Fout bij laden rollen.'); }
}
async function adminSave() {
  try {
    if (!(CURRENT_ROLE==='admin' || CURRENT_ROLE==='beheerder')) return;
    const uid = document.getElementById('adm-uid').value.trim(); if(!uid) { alert('Vul een UID in.'); return; }
    const rol = document.getElementById('adm-rol').value;
    const areas = Array.from(document.querySelectorAll('.adm-area')).filter(cb=>cb.checked).map(cb=>cb.value);
    await setDoc(doc(getFirestore(),'roles', uid), { rol, gebieden: areas }, { merge:true });
    alert('Opgeslagen.'); adminRefresh();
  } catch(e) { console.error('[adminSave]',e); alert('Opslaan mislukt.'); }
}
async function adminDelete() {
  try {
    ...
    alert('Verwijderd.'); document.getElementById('adm-uid').value=''; adminRefresh();
  } catch(e) { console.error('[adminDelete]',e); alert('Verwijderen mislukt.'); }
}
// bind knoppen (safe)
