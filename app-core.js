...
// Fix 222: zichtlijn (+cirkel) verbergen/tonen — bewust LOKAAL per toestel, niet gedeeld
...
function applyFilters(){
  ...
  allLines.forEach(line=>{
    const meta=line._meta||{}; const should = visiblePotIds.has(meta.potId) && !_hiddenLineIds.has(meta.id);
    // Lijn zelf
...
function _showDemoWelcome() {
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:28px 26px;width:360px;max-width:100%;box-shadow:0 12px 40px rgba(0,0,0,.3);text-align:center">
      ...
      <p style="font-size:13px;font-weight:700;color:#0f172a;margin:0 0 10px;text-align:left">
        Hoe wil je de app proberen?
      </p>
      <button id="demo-welcome-simple" style="width:100%;padding:13px;border-radius:10px;border:2px solid #0aa879;background:#f0fdf4;color:#0aa879;font-size:14px;font-weight:700;cursor:pointer;letter-spacing:.3px;margin-bottom:8px;display:flex;align-items:center;justify-content:center;gap:8px">
        🧩 Eenvoudige modus <span style="font-weight:400;font-size:12px">(stap voor stap)</span>
      </button>
      <button id="demo-welcome-expert" style="width:100%;padding:13px;border-radius:10px;border:none;background:#0aa879;color:#fff;font-size:15px;font-weight:700;cursor:pointer;letter-spacing:.3px">
        ⭐ Expert modus <span style="font-weight:400;font-size:12px">(volledige kaart)</span>
      </button>
      <div style="font-size:11px;color:#94a3b8;margin-top:8px">Kan je later altijd nog wisselen.</div>
  ...
  document.body.appendChild(modal);
  // Fix 220: modus-keuze — gebruikt de overlay-functies rechtstreeks i.p.v. te wachten op
  // de _swApplyMode() die verderop in de boot-flow draait, want die kan al gepasseerd zijn
  // tegen de tijd dat de gebruiker hier klikt.
  document.getElementById('demo-welcome-simple').addEventListener('click', () => {
    modal.remove();
    _swShowFromSidebar(); // zet override op 'simple' én toont de wizard meteen
  });
  document.getElementById('demo-welcome-expert').addEventListener('click', () => {
    modal.remove();
    _swSetOverride('expert');
    _swHide(); // zorgt dat de wizard-overlay zeker verborgen is, ook als die al actief was
  });
  modal.addEventListener('click', e => { if(e.target === modal) modal.remove(); });
}
